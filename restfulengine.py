...
class Report:
    ...
    def process(self, data_sources=None):
        ...
        if self.datasets is not None:
            req_json["Datasets"] = [ds.get_json() for ds in self.datasets]
...
class JsonDataSource(DataSource):
    """ DataSource implementation for JSON data sources.
    This data source must be instantiated with either JSON data or a URI
    from which to retrieve JSON data.  If both are specified, the data
    will be used.

    :param name: Name of this data source. This should match its
        name in the template.
    :type name: str
    :param data: JSON data as a bytestring.
    :type data: bytes
    :param uri: A URI from which to retrieve JSON data (such as a
        web URL or file location) -- note, this URI must be
        accessible from the RESTful service which is being called.
    :type uri: str
    :param domain: A domain name to use to access JSON data source.
    :type domain: str
    :param username: User credentials to access JSON data source.
    :type username: str
    :param password: User credentials to access JSON data source.
    :type password: str
    """
    ...
    def __init__(self,
        ...
        # enforce requirements
        if self._data is None and self._uri is None:
            raise ReportException("JsonDataSource objects must be instantiated"
                                  " with either a data string or a URI.")
    ...
    def get_json(self):
        ...
        if self._data is not None:
            ...
            json["Uri"] = self._uri
        # other parameters: domain, username, password
        if self._domain is not None:
            json["Domain"] = self._domain
...
class ODataDataSource(DataSource):
    """ DataSource implementation for OData data sources.
    This data source must be instantiated with a URI pointing to the
    OData service.
    ...
    :param password: User credentials to access JSON data source.
    :type password: str
    :param protocol: OData authentication protocol to use.  Use
        `Protocol` enum.
    ...
    def __init__(self,
        ...
        :type password: str
        :param protocol: OData authentication protocol to use.  Use
            `ODataDataSource.Protocol` enum.
        :type protocol: ODataDataSource.Protocol
        """
        DataSource.__init__(self, name)
        self._uri = uri
        ...
        if self._uri is None:
            raise ReportException("ODataDataSource objects must be instantiated"
                                  " with a URI!")
        if self._version not in [1, 2, 3, 4]:
            raise ReportException(str(self._version) + " is not a valid OData version number")
        if self._protocol not in ODataDataSource.Protocol:
            raise ReportException(self._protocol + " is not a valid OData authentication protocol.")
    ...
    def get_json(self):
        ...
        if self._version is not None:
            json["ODataVersion"] = self._version
        # other parameters: domain, username, password, protocol
        if self._domain is not None:
            json["Domain"] = self._domain
        if self._username is not None:
            json["Username"] = self._username
        ...
        if self._protocol is not None:
            json["ODataProtocol"] = self._protocol.value
...
class DataSet:
    ...
    DataSets must be instantiated with either JSON data or a URI
    from which to retrieve JSON data.  If both are specified, the data
    will be used.

    :param data: DataSet file (*.rdlx) data as a bytestring.
    :type data: bytes
    :param uri: A URI from which to retrieve DataSet file (such as a
        web URL or file location) -- note, this URI must be
        accessible from the RESTful service which is being called.
    :type uri: str
    """
    def __init__(self, data=None, uri=None):
        ...
        # enforce requirements
        if self._data is None and self._uri is None:
            raise ReportException("DataSet objects must be instantiated"
                                  " with either a data string or a URI.")

    def get_json(self):
        """ Generates and returns a JSON representation of this DataSet.
        This is intended to be used when packaging up all required
        information for a report request by :py:meth:`Report.process()`
        :return: JSON dict of this DataSet intended for a request
        :rtype: dict
        """
        json = {}
        if self._data is not None:
